...
library LibPosition {
        ...
        returns (Position rightIndex_)
    {
        ...
        assembly {
            ...
            // difference from left most to right most nodes at max depth
            // 2 ** rem * pos +  2**rem - 1
            // right 1 -> 2 * pos + 1
        ...
        returns (uint256 traceIndex_)
    {
        ...
        assembly {
            let remaining := sub(_maxDepth, msb)
            // right index - left most node at max depth
        ...
        returns (Position ancestor_)
    {
        ...
        // Position in binary encodes tree path
        //    1    p = 01
        //   / \
        //  2   3
        // 10   11 <- p << 1 + (0 or 1)
        // Given child c, to find parent p = c >> 1

        // Trace ancestor = highest ancestor that is left of pos
        //                = keep going up + left from child, until can't go left
        //                = keep dividing by 2 until even or 1
        //                = keep shifting right until last bit is 0
        //        1        <-     01
        //      /   \           /    \
        //     2     3     <-  10     11
        //    / \   / \       / \     / \
